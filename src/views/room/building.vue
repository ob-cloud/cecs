    ...
    <div class="building-list" v-loading.lock="loading"  :style="{height: (height - 80) + 'px'}">
      <div class="building-item" v-for="item in buildingList" :key="item.id">
        ...
        <div class="content">
          ...
          <p class="text">{{item.buildName}} {{$t('message.building')}}</p>
        </div>
      </div>
      <el-pagination
        class="pagination"
...
export default {
  ...
  data () {
    return {
      ...
      dialogStatus: '',
      dialogTitleMap: {
        edit: this.$t('smart.building.dialogTitle', {TITLE: 'edit'}),
        create: this.$t('smart.building.dialogTitle', {TITLE: 'create'})
      },
      searchModel: {
        buildName: '',
      ...
      creationRules: {
        buildName: [{ required: true, message: this.$t('message.rules', {RULE: 'build'}), trigger: 'blur' }]
      }
    }
  },
  created () {
  ...
  methods: {
    ...
    handleRemove (item) {
      this.$confirm(this.$t('smart.building.message', {MESSAGE: 'rmRoomConfirm'}), this.$t('message.tips'), {
        confirmButtonText: this.$t('message.confirm'),
        cancelButtonText: this.$t('message.cancel'),
        type: 'warning',
        closeOnClickModal: false
